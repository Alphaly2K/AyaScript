...
export class VariableSymbolTable {
    ...
    addEntryNV(name: string, value: any, isArray:boolean=false) {
        ...
        this.table.set(name, {
            name: name, isArray: isArray, type: (typeof value).toString(),
        });
    }

    addEntryFuncParam(name: string, value?: any, isArray:boolean=false,memoryAddress?: number) {
          this.table.set(name, { name: name, isArray: isArray, type: (typeof value).toString(),value,memoryAddress})
    }
